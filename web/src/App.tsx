...
function HomeView({ onAsk }: { onAsk: (q: string) => void }) {
  ...
  return (
    <div className="fade-in mx-auto flex min-h-[calc(100vh-3.5rem)] max-w-2xl flex-col items-center justify-center px-4 pb-24">
      <h1 className="mb-2 text-center text-4xl font-semibold tracking-tight sm:text-5xl">
      ...
      <form
        ...
        className="w-full"
      >
        <div className="flex items-center gap-2 rounded-2xl border border-border bg-white p-2 shadow-md focus-within:ring-2 focus-within:ring-foreground/10">
          ...
          <input
            ...
            className="flex-1 bg-transparent px-1 py-2 text-[15px] outline-none placeholder:text-muted"
          />
          <button
            type="submit"
            disabled={!value.trim()}
            className="grid h-9 w-9 place-items-center rounded-xl bg-foreground text-background disabled:opacity-30"
          >
      ...
      <div className="mt-5 flex flex-wrap justify-center gap-2">
        {SUGGESTIONS.map((s) => (
          <button
            key={s}
            onClick={() => onAsk(s)}
            className="rounded-full border border-border bg-white px-3.5 py-1.5 text-sm text-muted transition hover:text-foreground hover:shadow-sm"
          >
            {s}
          </button>
        ))}
          ...
          {messages.map((m, i) => (
            <div key={i} className={m.role === "user" ? "flex justify-end" : "flex justify-start"}>
              <div
                className={
                  m.role === "user"
                    ? "max-w-[85%] whitespace-pre-wrap rounded-2xl bg-foreground px-4 py-2.5 text-[15px] leading-[1.7] text-background"
                    : "max-w-[85%] whitespace-pre-wrap rounded-2xl border border-border bg-white px-4 py-2.5 text-[15px] leading-[1.75]"
                }
              >
                {m.content}
              </div>
        ...
        <div className="mx-auto max-w-3xl px-4 pb-5">
          <div className="pointer-events-auto mb-3 flex justify-center">
            <button
              ...
              className="flex items-center gap-2 rounded-full bg-foreground px-5 py-2.5 text-sm font-medium text-background shadow-lg transition hover:opacity-90 disabled:opacity-40"
            >
          ...
          <form
            ...
            className="pointer-events-auto flex items-center gap-2 rounded-2xl border border-border bg-white p-2 shadow-md"
          >
            <input
              value={value}
              onChange={(e) => setValue(e.target.value)}
              placeholder="Ask a follow-up…"
              className="flex-1 bg-transparent px-2 py-1.5 text-[15px] outline-none placeholder:text-muted"
            />
            <button
              type="submit"
...
function LoadingView() {
  ...
  const bar = "rounded-md bg-border/70 animate-pulse";
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <span
          ...
          style={{ backgroundColor: personaColor(comment.role) }}
        >
          <AgentIcon agent={comment.agent} role={comment.role} />
        </span>
        <span className="text-sm font-semibold">{comment.role}</span>
        {comment.verdict && (
          <span
      ...
      <div className="mt-3 flex items-center gap-3 text-xs text-muted">
        <button
          ...
          className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 transition ${
            voted ? "border-foreground text-foreground" : "border-border hover:text-foreground"
          }`}
        >
        ...
        >
          <div className="flex items-center gap-2 rounded-lg border border-border p-1.5">
            <input
              ...
              className="flex-1 bg-transparent px-1.5 text-sm outline-none placeholder:text-muted"
            />
            <button
              type="submit"
              disabled={!text.trim() || sending}
              className="grid h-7 w-7 place-items-center rounded-md bg-foreground text-background disabled:opacity-30"
            >
        ...
        <div className="mt-3 space-y-3">
          {thread.map((ex, i) => (
            <div key={i} className="space-y-1.5">
              <div className="ml-auto w-fit max-w-[90%] rounded-lg bg-foreground px-3 py-2 text-[13px] leading-[1.5] text-background">
                {ex.q}
              </div>
              <div className="rounded-lg bg-background p-3 text-[13px] leading-[1.6]">
  ...
  return (
    <div className="fade-in mx-auto max-w-6xl px-4 py-8">
      <div className="mb-6 flex items-center justify-between gap-3">
        ...
        <button
          onClick={onNew}
          className="inline-flex items-center gap-1.5 rounded-full border border-border bg-white px-3.5 py-1.5 text-sm text-muted hover:text-foreground"
        >
      ...
      <div className="grid gap-8 lg:grid-cols-[7fr_3fr]">
        {/* Article */}
        <article>
          <h1 className="mb-5 text-3xl font-semibold tracking-tight">{result.answer.title}</h1>
